...
**1. 您需要新的Maxthon浏览器才能访问最新版本的Vbox。**如果您当前正在使用其他浏览器，请使用 Maxthon 打开此链接并继续。如果没有安装 Maxthon ，可以访问 Maxthon 官网下载最新版本。Maxthon 移动端也可以注册Vbox云账号，您可以在官网下载并安装移动端 Maxthon 并按照后续指引操作。 Maxthon 官网地址为 https://www.maxthon.cn/

点击右上角Vbox的字母V![](images/11-2.png)图标，即可启动Vbox。

**2. 在地址栏右侧找到Vbox图标。如果您使用的是移动端 Maxthon，可以在浏览器启动页快速拨号栏的中间找到Vbox图标。![](images/11-2.png)**它看起来像一个V字。单击图标，您将在浏览区域的右侧看到Vbox页面。

**3. 单击“注册Vbox云账号”按钮。** 然后按照Vbox用户界面上的说明进行操作。

- 在启动页点击【注册Vbox云账户】按钮，按照引导注册即可获得一个区块链身份。
- 您可以使用手机号或邮箱两种方式注册，如果您使用手机号注册，您还可以获得一个免费的NB域名。免费域名一般为一个自增的六位数字。
- NB域名是区块链通用通行证，可以与任意地址绑定，从而成为固定的身份标识，作为登录、交易、转账的依据，并能为区块链的内容形成精简web链接。NB域名可以用于一些与 Maxthon 合作的区块链应用登录。
...
**4. 强烈建议您使用手机号码注册帐户。这样您可以直接得到一个免费NB域名。**即使您选择电子邮件注册，注册免费域名时，也需要提供手机号码。每个手机号在每一个区块链上，只能申请一个免费 NBdomain 域名。

**5. 注册完成后，您需要根据指引自定义ID备忘名和支付密码。**

**6. 如果您使用电子邮件注册，您可以也可以在“我”选项卡中点击 绑定NB域名 ，通过输入一个手机号和验证码获得免费NB域名。**请注意，绑定可能会有几分钟延迟。大约10分钟后，您的Vbox ID将绑定到新的NBdomain。

**7. 尽管每个Vbox帐户可以创建多个ID，但在每个区块链上，每个账户只能获得一次免费的NBdomain。**如果您需要另一个免费域名，请使用新的手机号码注册一个新的Vbox云帐户。

**8. 您还可以在NBdomain网站上注册永久个性化域名。**免费域名是一个随机的6位数字，使用NBdomain登录应用时，您的NB域名将会被显示成昵称。免费域名难以记忆和识别，如果想让大家记住您，最低只要十几块，就可以获得一个可以永久使用的个性化域名。https://www.nbdomain.com/#/
...
Maxhotn允许用户注册多个Vbox身份，每个Vbox身份都对应单独的私钥和钱包地址，且不可更改，解绑。
...
<font color=#A30014>*注意：在授权时，勾选【加入自动授权列表】，可在下一次执行授权时，不再唤起侧边页面手动授权，Vbox将在后台为您自动执行授权。</font>
...
Vbox的数据存储在本地，为保护数据安全，避免因错误操作而导致的数据丢失，Vbox提供了云同步功能。

![](images/11-22.png "=85%,85%")

- 在【我的页面】，点击【自动云备份】，登录Vbox云账户后可开启；
- 使用云备份，可在多设备上简单地同步身份数据。
...
<font color=#A30014>*注意：如果在清除数据时开启了云备份，弹窗将向您确认是否同步删除云备份，如勾选，则云账户的全部数据将被同步删除，如未勾选，则云备份功能将被自动关闭，然后再清理本地数据。</font>
